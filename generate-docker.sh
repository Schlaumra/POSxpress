...
docker buildx build --platform linux/arm64 --no-cache -t posxpress dist/docker/posxpress
...
docker buildx build --platform linux/arm64 --no-cache -t posxapi dist/docker/posxapi
